...
public class EnumToBooleanConverter : IValueConverter
{
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (parameter is not string enumString)
            throw new ArgumentException($"{nameof(parameter)} must be an enum name", nameof(parameter));
        ...
        if (!Enum.TryParse(typeof(ApplicationTheme), enumString, out var enumValue))
            throw new ArgumentException($"'{enumString}' is not a valid {nameof(ApplicationTheme)} value", nameof(parameter));
